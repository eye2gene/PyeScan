...
def _validate_dataframe_integrity(
    df,
    identity_col=None,
    bscan_index_col="bscan_index",
    ...
    for group_id, df_group in df.groupby(source_id_col):
        ...
        if (
            identity_col
            and identity_col in df.columns
            and identity_col != source_id_col
        ):
            identity_values = df_group[identity_col].unique()
            identity_label = (
                f"{id_col}={identity_values[0]}, {source_id_col}={group_id}"
            )
            ...
            if len(bscan_indices) > 0:
                ...
                if indices != expected:
                    ...
                    if missing:
                        ...
                        shown = missing[:10]
                        msg_parts.append(
                            f"missing indices {shown}{'...' if len(missing) > 10 else ''}"
                        )
                    if extra:
                        shown = extra[:10]
                        msg_parts.append(
                            f"unexpected indices {shown}{'...' if len(extra) > 10 else ''}"
                        )
                    if msg_parts:
                        errors.append(
        ...
        if "image_number" in view_info:
            ...
            if "bscan_index" in df.columns and len(df) > 0:
                ...
                if col_dtype == object:  # string column
                    ...
                    image_number = col_dtype.type(image_number)
...
def load_records_from_CE(
    path_to_records_folder, folder_structure="{pat}/{sdb}/metadata.json"
):
    raise NotImplementedError()


def load_record_from_df(
    df_scan, column_headings=None, identity_col=None, validate=True
):
    ...
    if not isinstance(df_scan, pd.DataFrame):
        raise TypeError(f"Expected a pandas DataFrame, got {type(df_scan).__name__}.")
    if df_scan.empty:
        raise ValueError(
...
def load_records_from_df(
    df, column_headings=None, identifier_columns=None, identity_col=None, validate=True
):
