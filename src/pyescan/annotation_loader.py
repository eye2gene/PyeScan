...
def _validate_annotation_dataframe(
    ...
    for group_id, df_group in groups:
        ...
        if isinstance(group_id, tuple):
            label = ", ".join(
                f"{col}={val}" for col, val in zip(group_cols, group_id, strict=False)
            )
        elif group_cols:
...
def _build_annotation_from_file_paths(
    file_paths,
...
def load_annotation_from_df(
    ...
    if feature_col:
        ...
        for feature, df_feat in df.groupby(feature_col):
            ann = _build_annotation_from_dataframe_base(
                df_feat, file_path_col, index_col
            )
...
def load_annotation_from_folder(
    annotations_folder,
    folder_structure=r"{feature}/{source_id}_{bscan_index:\d+}.png",
    group_by=None,
    identity_col=None,
    validate=True,
    allow_gaps=False,
    ...
    df = summarise_dataset(
        annotations_folder, structure=folder_structure, progress=False
    )
