...
class AnnotationSlice:
    ...
    def __init__(
        self,
        raster: NDArray | None = None,
        contours: list[NDArray] | None = None,
    ...
    @property
    def contours(self) -> list[NDArray] | None:
        return self._contours

    @property
    def points(self) -> NDArray | None:
        return self._points
...
class AnnotationVolume(ArrayView):
    ...
    def data(self) -> NDArray:
        ...
        stacked = [
            a if a is not None else np.zeros(shape, dtype=np.uint8) for a in arrays
        ]
    ...
    def images(self):
        ...
        return ImageVolume(
            images=[
                LazyImage(raw_image=img) if img else LazyImage() for img in pil_images
            ]
        )
...
class Annotation:
    ...
    def _build_display_widget(self):
        from .visualisation import enface_display_widget, image_array_display_widget
...
class MaskVolume(ArrayView):
    ...
    def data(self) -> NDArray:
        ...
        arrays = [
            np.array(m.image) if m.image is not None else None for m in self._masks
        ]
        ...
        if masks is not None and slices is None:
            if isinstance(masks, MaskVolume):
                ...
                for mask_img in masks._masks:
                    ann_slices.append(
                        AnnotationSlice(
                            file_path=mask_img._file_location
                            if isinstance(mask_img._file_location, str)
                            else None,
                            raster=np.array(mask_img._raw_image)
                            if mask_img._raw_image is not None
                            else None,
                        )
                    )
