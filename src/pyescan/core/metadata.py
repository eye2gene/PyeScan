...
class MetadataRecord:
    ...
    def __init__(self, data: Any, location: str | None = None, parser=None):
        self._format = None
        self._data = data
...
class MetadataView:
    ...
    @property
    def parser(self) -> MetadataParser | None:
        return self._parser or self._record._parser

    def __getattr__(self, attribute_name: str) -> Any:
        # Only called when normal attribute lookup fails
        parser = (
            object.__getattribute__(self, "_parser")
            or object.__getattribute__(self, "_record")._parser
        )
        if parser is None:
    ...
    def get_view(self, view_info: dict[str, Any] | None) -> "MetadataView":
    ...
    def images(self) -> list["MetadataView"]:
        return [
            self.get_view({"scan_number": self.scan_number, "image_number": i})
            for i in range(int(self.n_images))
        ]
...
class MetadataParserJSON(MetadataParser):
    ...
    def _map_path(
        self, path: list[str | int] | None, view_info: dict[str, Any] | None
    ) -> list[str | int] | None:
        mapped_path = list()
        for p in path:
...
class MetadataParserCSV(MetadataParser):
    @abstractmethod
    def _get_records_subset(
        self, metadata_record: MetadataRecord, view_info: dict[str, Any] | None
    ):
        return NotImplementedError()

    def get_value(
        self, attribute_name: str, metadata_record: MetadataRecord, view_info
    ) -> Any:
        if attribute_name in self._overrides:
            return self._overrides[attribute_name](metadata_record, view_info)
