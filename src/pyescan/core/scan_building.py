...
def resolve_loader(
    location: str, mode: str | None = None
...
def build_faf_from_metadata(scan_meta, image_loader=None):
    ...
    scan_img_path = _get_metadata_attr(
        scan_meta, "image_location", context=f"FAF scan source_id={source_id}"
    )
...
def build_ir_from_metadata(scan_meta, image_loader=None):
    ...
    scan_img_path = _get_metadata_attr(
        scan_meta, "image_location", context=f"IR scan source_id={source_id}"
    )
    scan_img = LazyImage(loader=image_loader(scan_img_path))
...
def build_from_metadata(metadata, image_loader=None):
    ...
    for group in groups:
        try:
            modalities = [
                _get_metadata_attr(scan, "modality", context="scan group")
                for scan in group
            ]
        ...
        try:
            if modalities == ["SLO - Infrared", "OCT"]:
                ...
                scan = build_oct_from_metadata(
                    oct_meta, ir_meta, image_loader=image_loader
                )
                scans.append(scan)
                ...
                oct_meta = group[0]
                scan = build_oct_from_metadata(
                    oct_meta, None, image_loader=image_loader
                )
                scans.append(scan)
                ...
                logger.warning(
                    f"Skipping unrecognised scan group with modalities: {modalities}"
                )
