...
def image_array_display_widget(images, width=320, height=320, return_slider=False):
    ...
    w_image_volume = widgets.Image(
        value=encoded_volume[n_images // 2], width=width, height=height
    )
...
def draw_bscan_lines(enface_image, bscan_positions, bscan_index=None, draw_pos=False):
    ...
    for i, bscan_position in enumerate(bscan_positions):
        ...
        pos = (
            (bscan_position[0][0], bscan_position[0][1]),
...
def oct_display_widget(
    ...
    w_image_enface = widgets.Image(
        value=encoded_enface, width=enface_size, height=enface_size
    )

    # Get widget for displaying volume
    w_image_volume, w_slider = image_array_display_widget(
        images, width=width, height=height, return_slider=True
    )

    if bscan_locations is not None:
...
def overlay_masks(
    image, masks, colors=None, feature_names=None, alpha=0.5, max_height=None
):
...
def render_volume_data(
    data, color=(255, 0, 0), heatmap=True, contours=True, figsize=(10, 10)
):
    ...
    cmap = mcolors.LinearSegmentedColormap.from_list(
        "custom", [(0, 0, 0), np.array(color) / 255]
    )
