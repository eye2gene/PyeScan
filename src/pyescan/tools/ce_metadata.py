...
def parse_ce_metadata_json(
    metadata_path: str,
    identifier_dict: dict[str, str] | None = None,
...
def scrape_ce_export(
    ...
    df_meta_files = summarise_dataset(
        export_location, structure=file_structure, progress=progress
    )
...
def _validate_export(
    df: pd.DataFrame, on_missing: str = "warn", on_duplicate: str = "warn"
):
    ...
    if "scan_uid" in df.columns and "bscan_index" in df.columns:
        ...
        if dup_mask.any():
            ...
            msg = f"Found {n_dups} duplicate (scan_uid, bscan_index) entries:\n{dup_examples}"
