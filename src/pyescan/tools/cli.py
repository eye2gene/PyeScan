...
@click.option(
    ...
    type=str,
    default="pat/sdb",
    help="File structure gias list of names separated by / (default: pat/sdb).",
)
@click.option(
    "--skip_image_level/--include-image-level",
    default=False,
    help="Skip image level information for faster parsing (default: False).",
)
def get_ce_export_summary_cli(
    export_location,
    output_csv,
    file_structure="pat/sdb",
    merged=True,
    skip_image_level=False,
):
...
@click.option(
    "--format",
    type=str,
    default=r"{pat}/{sdb}/{source_id}_{bscan_index:\d+}.png",
...
def run_function_on_csv_cli(csv_file, function, output_csv=None, skip_prompt=False):
    ...
    """
    if not output_csv:
        confirmation = skip_prompt or click.confirm(
            f"No output location specified. Are you sure you want to overwrite {csv_file}?"
        )
        if confirmation:
            output_csv = csv_file
        else:
...
@click.option(
    ...
    "-c",
    type=click.STRING,
    help="Column headings separated by commas.",
)
@click.option(
    "--threaded", is_flag=True, help="Use threading (recommended, requires pathos)."
)
@click.option("--skip_prompt", "-y", is_flag=True, help="Skip confirmation.")
def run_function_over_csv_cli(
    csv_file,
    function,
    output_csv=None,
    ...
    if len(column_headings) == 1:
        column_headings = column_headings[0]
    ...
    result_df = run_function_on_dataframe(
        df, function_to_apply, column_headings, threaded
    )
...
@click.option(
    "--mapping",
    type=ClickDictParamType(),
    help="Mapping from CSV column headings to internal ",
)
@click.option(
    "--suffix", "-s", type=click.STRING, help="Suffix to append to added column names."
)
@click.option(
    "--intermediates",
    is_flag=True,
    help="Save intermediate metrics calculated in process.",
)
@click.option(
    "--threaded", is_flag=True, help="Use threading (recommended, requires pathos)."
)
@click.option("--skip_prompt", "-y", is_flag=True, help="Skip confirmation.")
def run_metric_csv_cli(
    csv_file,
    stats,
    output_csv=None,
    mapping={},
...
@click.option(
    ...
    is_flag=True,
    default=True,
    help="Flatten names of column headings.",
)
@click.option("--skip_prompt", "-y", is_flag=True, help="Skip confirmation.")
def narrow_to_wide_cli(
    csv_file,
    output_csv,
    pivot_col,
    identifier_cols,
