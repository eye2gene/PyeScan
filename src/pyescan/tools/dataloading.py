...
class SegmentationDataLoader:
    ...
    def _get_transform_matrix(self, in_size, out_size, rotate, scale, translate):
        ...
        cy = h / 2
        cx = w / 2
        C1 = np.array([[1, 0, -cx], [0, 1, -cy], [0, 0, 1]], dtype=np.float)
        ...
        # scale
        sy, sx = scale
        S = np.array([[sx, 0, 0], [0, sy, 0], [0, 0, 1]], dtype=np.float)

        # top left corner to center
        h, w = out_size
        ty = h / 2
        tx = w / 2
        C2 = np.array([[1, 0, tx], [0, 1, ty], [0, 0, 1]], dtype=np.float)

        # translate
        ty, tx = translate
        T = np.array([[1, 0, tx], [0, 1, ty], [0, 0, 1]], dtype=np.float)

        return T @ C2 @ S @ R @ C1

    def _get_image_patch(
        self, image, labels, patch_size, rotate=0.0, scale=1.0, translate=(0.0, 0.0)
    ):
        ...
        M = self._get_transform_matrix(
            image.shape, patch_size, rotate, scale, translate
        )
    ...
    def _intensity_transform(self, image, brightness=0.0, contrast=1.0, gamma=1.0):
        return np.clip((contrast * image**gamma + brightness), 0, 1)

    def _process_sample(
        self, raw_image, raw_labels, augment=False, image_scaling_factor=(1.0, 1.0)
    ):
        ...
        image, labels = self._get_image_patch(
            raw_image, raw_labels, self.patch_size, rotation, scale, (tx, ty)
        )
    ...
    def _get_scale_factor(self, scan_resolutions_mm_width, scan_resolutions_mm_height):
        scale_x = (
            scan_resolutions_mm_width
            / self.targ_resolutions_mm_width
            * self.scale_factor
        )
        scale_y = (
            scan_resolutions_mm_height
            / self.targ_resolutions_mm_height
            * self.scale_factor
        )
    ...
    def _load_image_from_row(self, row, features):
        ...
        labels_paths = [row[f"file_path_{feature}"] for feature in features]
    ...
    def _load_data_from_row(self, row, features, augment=False):
        ...
        scale_factor = self._get_scale_factor(
            row.resolutions_mm_width, row.resolutions_mm_height
        )
        image, labels = self._process_sample(
            raw_image, raw_labels, augment, image_scaling_factor=scale_factor
        )
        return image, labels


def __get_default_scaling_from_row(row, scale_factor=1.0, target_resolution=None):
...
def __load_processed_data_from_row(
    row, features, patch_size, augment=False, target_resolution=None
):
    ...
    image, labels = get_patch(
        raw_image, raw_labels, patch_size, rotation, scale, (tx, ty)
    )
...
def __scale_to_target_resolution(
    input_image, output_dimension, image_spacing, target_spacing
):
