...
class TransferMetric:
    ...
    width: int = 768
    height: int = 768
    skew_weight: float = 0.0  # optional similarity prior, scaled to ~pixel units

    default_f_scale: ClassVar[float] = (
        2.0  # px; residuals beyond this are down-weighted
    )
...
SIMILARITY = PoseModel(
    "similarity", 4, _encode_similarity, _decode_similarity
)  # no perspective, no skew
...
class TransformGraph:
    ...
    def __init__(
        ...
        confidences: dict[Edge, float] | None = None,
        failed: list | None = None,
    ):
        self.edges: Transforms = {
            k: np.asarray(M, float) / M[2, 2] for k, M in transforms.items()
        }
            ...
            try:
                ...
                if confidence_fn is not None:
                    with contextlib.suppress(Exception):
                        confidences[img_path_0, img_path_1] = float(
                            confidence_fn(transform_result)
                        )
    ...
    def edge_key(self, src: Node, tgt: Node) -> tuple[Edge, bool] | None:
    ...
    def compose_path(self, path) -> Homography | None:
    ...
    def residual_errors(
        self, poses: Poses, metric: Metric = DEFAULT_METRIC
    ) -> dict[Edge, float]:
        ...
        for (src, tgt), M in self.edges.items():
            ...
            errors[src, tgt] = np.linalg.norm(
                metric.residuals(M_pred @ np.linalg.inv(M))
            )
        return errors
    ...
    def score_edges(
        self, metric: Metric = DEFAULT_METRIC, agg="min", quad_fallback=True
    ) -> ScoredEdges:
        ...
        edge_scores = {
            edge: (agg_fn(edge_errors[edge]) if edge in edge_errors else None)
            for edge in self.edges
        }
    ...
    def filter_outlier_edges(
        self, scored: ScoredEdges, threshold=10.0, use_quad_cycles=False
    ) -> FilteredGraph:
        ...
        eligible = {
            e: s
            for e, s in scored.scores.items()
            if s is not None and (use_quad_cycles or scored.source.get(e) == "triangle")
        }
        ...
        if threshold == "auto":
            ...
            if len(vals) >= 10:
                ...
                threshold_used = 10.0  # too few edges for robust stats
        else:
    ...
    def _spanning_tree(
        self, root: Node, component: set, edge_costs=None, default_cost=1.0
    ):
    ...
    def initialize_poses(self, method="dijkstra", edge_costs=None) -> InitialPoses:
        ...
        if costs is not None:
            ...
            default_cost = float(np.median(finite)) if finite else 1.0
    ...
    def refine_poses(
        ...
        pose_model: PoseModel | str = HOMOGRAPHY,
    ) -> Poses:
        ...
        sparsity = lil_matrix(
            (len(edge_data) * block, len(free) * model.dof), dtype=int
        )
...
def find_centroid_pose(
    ...
    def residuals(x):
        ...
        return np.concatenate(
            [metric.residuals(pose @ centroid_inv) for pose in poses.values()]
        )
...
def center_poses(
    ...
    if target_scale is not None:
        ...
        centroid = np.diag([k, k, 1.0]) @ centroid
...
class PipelineResult(Mapping):
    ...
    pose_scales: dict
    scale_clusters: list | None  # None if cluster_scales=False
    target_scale: float | None
    transform_magnitudes: dict = (
        None  # edge -> metric size of raw measurement (pre-filtering)
    )
    metric: Any = None
    pose_model: Any = None
    ...
    def summary(self):
        ...
        print(
            f"[cycles] {n_edges} edges: {n_tri} triangle-scored, {n_quad} quad-scored "
            f"(diagnostic), {len(self.untestable_edges)} unverified"
        )
        tested = [
            s
            for e, s in self.edge_scores.items()
            if s is not None and self.edge_score_source.get(e) == "triangle"
        ]
        if tested:
            print(
                f"[cycles] triangle scores: median={np.median(tested):.3f}, "
        ...
        print(
            f"[graph]  {len(self.components)} connected component(s), sizes={sizes}, "
        ...
        for label, errs in (
            ("init", self.initial_edge_errors),
            ("final", self.final_edge_errors),
        ):
            if errs:
                vals = list(errs.values())
                print(
                    f"[optimize] {label} edge residuals: "
        ...
        if self.final_edge_errors:
            ...
            print(
                f"[optimize] worst edge: {worst} ({self.final_edge_errors[worst]:.3f})"
            )

        if self.scale_clusters is not None:
            reps = [
                float(np.median([self.pose_scales[k] for k in cl]))
                for cl in self.scale_clusters
            ]
            print(
                "[scale]  clusters (size@scale): "
                + ", ".join(
                    f"{len(cl)}@{r:.3f}"
                    for cl, r in zip(self.scale_clusters, reps, strict=False)
                )
...
def solve_poses(
    ...
    *,
    ...
    graph = (
        transforms
        if isinstance(transforms, TransformGraph)
    ...
    # --- Pose refinement ---
    optimized = clean_graph.refine_poses(
        init_poses,
        metric,
        init.anchors,
        f_scale=f_scale,
        max_nfev=max_nfev,
        use_confidences=use_confidences,
        pose_model=model,
    )
    ...
    node_poses, centroid = center_poses(
        optimized, metric, target_scale, pose_model=model
    )

    result = PipelineResult(
        poses=node_poses,
        graph=clean_graph,
        initial_poses=init.poses,
        centroid=centroid,
        edge_scores=scored.scores,
        edge_errors=scored.errors,
        edge_score_source=scored.source,
        outlier_edges=filtered.outliers,
        untestable_edges=scored.untestable,
        ...
        transform_magnitudes=transform_magnitudes,
        metric=metric,
        pose_model=model,
    )
    if verbose:
...
def plot_edge_matrix(
    values, nodes=None, ax=None, title=None, log=True, mark_edges=None, cbar_label=None
):
...
def plot_pose_graph(
    ...
    graph = (
        graph_or_transforms
        if isinstance(graph_or_transforms, TransformGraph)
    ...
    vmax = max(errs.values()) if errs else 1.0
