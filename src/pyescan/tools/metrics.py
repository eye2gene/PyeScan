...
def get_mask_area(row):
    ...
    area = (
        row.pixel_count
        * row.resolutions_mm_width
        * row.resolutions_mm_height
        * rescale_w
        * rescale_h
    )
    horizontal_extent = (
        row.horizontal_pixel_count * row.resolutions_mm_width * rescale_w
    )
...
def _get_distance_mask_enface(row, radius):
    ...
    if not row.resolutions_mm_width:
        return 0
...
def _get_distance_mask_oct(row, radius):
    ...
    fovea_location = (
        row.fovea_x * row.resolutions_mm_width,
        row.fovea_bscan_index * row.resolutions_mm_depth,
    )

    intersection = _get_circle_line_intersection(
        fovea_location, radius, bscan_start, bscan_end
    )
...
def _infer_fovea_enface_position(row):
    ...
    bscan_start_x, bscan_start_y = (
        row.bscan_location_start_x,
        row.bscan_location_start_y,
    )
...
def get_pixel_count_at_distance(
    row, radius, hextent_only=False, by_quadrant=False, mask="NotProvided"
):
    import numbers
    ...
    if mask == "NotProvided":
        ...
        if not d_mask.any():
            return 0.0 if return_single_value else [0.0] * return_values_len
        if d_mask.all():
        ...
        mask = _load_image_as_mask(row.file_path)
        if mask is None:
            return 0.0 if return_single_value else [0.0] * len(d_masks)
...
def get_distance_volumes(row, distances=[0.5, 1.5, 3.0], by_quadrant=False):
    ...
    results = get_pixel_count_at_distance(
        row, distances, hextent_only=False, by_quadrant=by_quadrant
    )
...
def get_distance_areas(row, distances=[0.5, 1.5, 3.0], by_quadrant=False):
    ...
    results = get_pixel_count_at_distance(
        row, distances, hextent_only=hextent_only, by_quadrant=by_quadrant
    )
    return [r * scale for r in results]
