...
from pyescan.core.image import ImageVolume, LazyImage
...
class TestArrayView:
    ...
    def test_len(self):
        items = [
            LazyImage(raw_image=PILImage.fromarray(np.zeros((4, 4), dtype=np.uint8)))
            for _ in range(3)
        ]
    ...
    def test_slice(self):
        items = [
            LazyImage(raw_image=PILImage.fromarray(np.full((4, 4), i, dtype=np.uint8)))
            for i in range(5)
        ]
        vol = ImageVolume(images=items)
        sliced = vol[1:3]
